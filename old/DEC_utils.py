...
def get_waypoint_data(flight, waypoint_type='a'):
    ...
    data = pd.read_table(wpfile, names=columns, skiprows=3, engine='python',
                         delim_whitespace=True, date_parser=parseFunc, index_col='lab',
...
def bmap(ax=None, drawlines=True, llr=None, par_labs=[1, 1, 0, 0], mer_labs=[0, 0, 1, 1], merspace=15, parspace=15, 
         **kwargs):
    ...
    if drawlines:
        m.drawparallels(np.arange(-90., 90., parspace), labels=par_labs)
        m.drawmeridians(np.arange(-180., 180., merspace), labels=mer_labs)
    m.drawcoastlines()
